...
- 2026-06-05 (диагональные блинки WebEngine в редакторе кривых): UI — `appPROEKT1/PROEKT1.py`, дизайн — `appDESIGN/design-preview.html`, профильный MD — `appPROEKT1/ROUNDED-CORNERS-FIX.md`. Свежий `appPROEKT1/ui-debug.log` и скрин показали не JS-крэш и не `renderProcessTerminated`, а повреждение Chromium compositor-layer внутри прозрачного frameless `QWebEngineView`: при быстром движении над графиком или после reset/max-points появлялся диагональный тёмный слой, иногда зависавший до следующего repaint. Исправлено: hover-превью графика переведено с частых SVG/DOM-мутаций на canvas overlay; SVG-точки исключены из browser hit-testing; QWebEngine стартует с `QTWEBENGINE_CHROMIUM_FLAGS` для software compositing (`--disable-gpu`, `--disable-gpu-compositing`, `--disable-gpu-rasterization`, `--disable-zero-copy`, `--disable-accelerated-2d-canvas`, `--disable-webgl`), а `QWebEngineSettings` выключает `WebGLEnabled` и `Accelerated2dCanvasEnabled`. Для сценария restore/minimize добавлен `_recover_web_view_after_show()` — paint-only восстановление WebView без reload и изменения размеров.

- 2026-06-03 (high-DPI drag без resize feedback и быстрый локальный старт): UI — `appPROEKT1/PROEKT1.py`, дизайн — `appDESIGN/design-preview.html`, правила — `PROEKT1 rools.md`, профильный MD — `appPROEKT1/ROUNDED-CORNERS-FIX.md`. Свежий `appPROEKT1/ui-debug.log` после предыдущей правки показал регрессию: live resize-pin на DPR>1 разгонял WebEngine chain `1375×950 → 2750×1900 → 5500×3800`, из-за чего интерфейс мерцал, сдвигался влево и снова показывал углы до release. Исправлено: live size-pin полностью запрещён; во время DPR>1 drag разрешён только position-only repair (`SWP_NOSIZE`, `x=0,y=0`) для child/native-chain HWND, а `_web_view_expected_native_metrics()` для HRGN принимает фактический текущий client rect, если Chromium ещё не стал physical-size. Runtime CSS в обычном оконном режиме оставляет `border:0`, поэтому двойная рамка на `125%` остаётся исправленной. Для ускорения старта удалён внешний `@import` Google Fonts из `design-preview.html`; WebView получает начальную геометрию сразу по размеру окна, а `interface.scale.apply` не запускает native pin/recovery, пока WebView скрыт за startup overlay.

- 2026-06-03 (smooth mixed-DPI drag без прыжков/half-size WebView): UI — `appPROEKT1/PROEKT1.py`, правила — `PROEKT1 rools.md`, профильный MD — `appPROEKT1/ROUNDED-CORNERS-FIX.md`. Свежий `appPROEKT1/ui-debug.log` показал новую регрессию после предыдущего фикса: при переносе 2→1 окно оставалось `1100×760`, но `QWebEngineView`/native HWND закреплялся как `550×380`, после чего `_sync_actual_web_fit_factor()` принимал это за настоящий `dpiFit=0.5` и дальнейшие `web.native_chain.pin` только консервировали маленький интерфейс в левом верхнем углу серой подложки. На обратном переносе 1→2 live `SetWindowPos` мог временно закреплять WebView как `2200×1520`, что давало мерцание. Исправлено: `_web_view_expected_native_metrics()` и `_actual_web_fit_factor()` берут логическую базу из `centralWidget().rect()`, а не из уже поплывшего WebView; `_set_web_view_geometry_stable()` и `_pin_web_view_native_chain()` возвращают Qt-геометрию WebView к central rect после native pin. Во время удержания ЛКМ native-chain pin запрещён, работает только `SetWindowRgn`/HRGN для углов, а `SetWindowPos`-pin откладывается до release/verify. `_web_view_native_surface_status()` больше не считает surface исправной по одному hit-test: размер/позиция HWND должны совпадать с main client geometry. После выхода из Web fullscreen удалены таймеры `_force_geometry_nudge()` на 80/260 мс, чтобы не было видимого 1px подпрыгивания frame.

- 2026-06-03 (без прыжка рамки и углов при переносе 2->1): UI — `appPROEKT1/PROEKT1.py`, правила — `PROEKT1 rools.md`, профильный MD — `appPROEKT1/ROUNDED-CORNERS-FIX.md`. Свежий `appPROEKT1/ui-debug.log` показал две оставшиеся причины: после отпускания ЛКМ debounced `dpi.refresh.apply_begin` вызывал `dpi.apply.resize` и менял окно между `1088×752` и `1100×760`; при переносе с DPR=2 на DPR=1 `web.rgn.apply.done` брал stale Win32 client rect `2176×1504` при `dpi=96`, поэтому HRGN округлял область вдвое больше видимого WebView и прямоугольные углы были видны до следующего pin. Исправлено: `_refresh_dpi_dependent_layout()` больше не ресайзит top-level окно при monitor/DPI refresh, только пересинхронизирует WebView; `_web_view_expected_native_metrics()` принимает native/client rect только если его scale совпадает с текущим ожидаемым DPR, где `GetDpiForWindow()` имеет приоритет над временно stale Qt DPR, иначе строит размер из logical WebView × current DPR. Уточнено следующей записью: live native-chain pin во время удержания ЛКМ не применять; во время drag переустанавливать только HRGN, а `SetWindowPos`-pin делать после release.

- 2026-06-03 (уголки во время drag полностью запрещены): UI — `appPROEKT1/PROEKT1.py`, правила — `PROEKT1 rools.md`, профильный MD — `appPROEKT1/ROUNDED-CORNERS-FIX.md`. Свежий `appPROEKT1/ui-debug.log` показал точную причину видимых углов: на `native_drag.start` код вызывал `web.rgn.suppress`, делал `SetWindowRgn(web_hwnd, 0, TRUE)`, и `windowRgn` становился `ERROR_OR_NONE`; пока ЛКМ удерживалась, прямоугольный Chromium backing был виден за 24px Qt-силуэтом, а после release HRGN возвращался и углы исчезали. Исправлено: видимый normal-mode `QWebEngineView` больше не очищает HRGN при drag/screenChanged/interface-scale; `_suppress_web_view_native_round_rgn()` игнорируется для видимой поверхности и сразу переустанавливает top-level клип. `_schedule_web_view_native_round_rgn_reapply()` больше не плодит рекурсивные `.deferred` таймеры во время удержания ЛКМ, а live-переустанавливает регион. Внутренние Chromium child-HWND по-прежнему не клипуются.

- 2026-06-02 (стабилизация 80%, углов и mixed-DPI без reload): UI — `appPROEKT1/PROEKT1.py`, правила — `PROEKT1 rools.md`. По свежему `appPROEKT1/ui-debug.log` найдено две новые причины: после `80%` на `\\.\DISPLAY1` Chromium давал полностью чёрный compositor surface и recovery пересоздавал WebEngine, а после серии переносов `native_surface.verify` считал поверхность исправной по Win32 client-origin, хотя визуально оставался только левый фрагмент интерфейса и большой тёмный viewport. Исправлено: для масштаба ниже `100%` на high-DPI включён `QWebEngineView.setZoomFactor(ui_scale)` вместо CSS `transform:scale(ui_scale)`; runtime `.app-window` снова держит канонические `1100×760px` вместо `vw/vh`, чтобы physical viewport Chromium не растягивал окно в огромную неинтерактивную поверхность. Recovery больше не делает немедленный `_recreate_web_engine_view()` при blank-check: сначала soft rebuild/pin/repaint и ожидание, чтобы вкладка не мигала и не сбрасывалась. Для углов найден профильный `appPROEKT1/ROUNDED-CORNERS-FIX.md`: прямоугольный Chromium HWND клипуется через top-level HRGN и на high-DPI; по уточнению 2026-06-03 видимый normal-mode регион больше нельзя очищать даже во время native drag/surface rebuild, внутренние child-HWND не клипуются.

- 2026-06-02 (80% без мерцания и сброса вкладки): UI — `appPROEKT1/PROEKT1.py`. Повторный `appPROEKT1/ui-debug.log` показал улучшение и новый побочный эффект: после `80%` на `\\.\DISPLAY1` recovery корректно обнаруживал полностью чёрную WebEngine-поверхность (`web.visual_blank.check`, все samples `#000000`) и пересоздавал WebEngine, но во время загрузки нового view native surface на один кадр попадал в координаты главного экрана (`clientToScreen0 x=380,y=124`), а вкладка возвращалась на default dashboard. Исправлено в источнике: для масштаба ниже `100%` WebUI больше не отдаёт Chromium `CSS zoom:0.8`; вместо этого `QWebEngineView.setZoomFactor()` остаётся `1.0`, CSS `--proekt1-ui-scale` остаётся `1`, а уменьшение уходит в `--proekt1-dpi-fit` / `transform:scale(0.8)`, тот же путь что у forced down-fit. Recovery смягчён: при blank сначала выполняется surface rebuild без reload HTML, recreate остаётся только последним fallback. `loadFinished` на high-DPI больше не делает opacity/refresh surface на скрытом WebView, а закрепляет native chain и обновляет виджет. Добавлено состояние `_web_active_view` + `window.PROEKT1.showView()`: nav/settings/curves сохраняют текущую вкладку в Python, а после любого reload/recreate `_sync_web_snapshot()` возвращает пользователя на ту же вкладку, включая настройки.

- 2026-06-02 (recovery серого WebEngine после 80% на втором экране): UI — `appPROEKT1/PROEKT1.py`. По свежему `appPROEKT1/ui-debug.log` сценарий `interface.scale.apply percent=80` на `\\.\DISPLAY1` не давал `renderProcessTerminated`, `ok=false` или `web.recreate`, а native HWND WebEngine оставался правильно прикреплённым к окну (`1760×1216` при DPR=2). Значит серый экран был не крашем и не прыжком HWND, а пустой Chromium compositor surface после уменьшения CSS-масштаба. Добавлен recovery для `uiScale < 100%` на high-DPI/mixed-DPI: после применения размера выполняется stable geometry + native pin + лёгкий compositor kick без opacity-hack, затем пиксельный visual-check WebView. Если WebView всё ещё выглядит как почти однотонный тёмный фон без текста/акцентов, WebEngine пересоздаётся через существующий `_recreate_web_engine_view()` и заново синхронизирует snapshot/settings. Этот же recovery вызывается после DPI-refresh при переносе окна между экранами, чтобы 80% и другие будущие размеры ниже 100% не оставляли приложение в сером экране.

- 2026-06-02 (добавлен размер интерфейса 80%): UI — `appDESIGN/design-preview.html`, `appPROEKT1/PROEKT1.py`. В dropdown `Размер интерфейса` добавлен пункт `80%`; whitelist WebUI/Python расширен до `80/100/125/150/200%`. `_interface_scale_factor()` и синхронизация CSS `--proekt1-ui-scale` теперь допускают нижнюю границу `0.8`, включая стартовую инициализацию runtime CSS после загрузки WebEngine. Сохранение/загрузка остаются через `config.json` ключ `interface_scale`, `QWebEngineView.setZoomFactor()` остаётся `1.0`, масштабирование работает тем же чётким CSS `zoom` + forced down-fit через `--proekt1-dpi-fit`.

- 2026-06-02 (стабилизация WebEngine при mixed-DPI переносе и fullscreen): UI — `appPROEKT1/PROEKT1.py`. По `appPROEKT1/ui-debug.log` найден оставшийся визуальный хвост без краша: во время `200%` + fullscreen на `\\.\DISPLAY1` Qt/WebEngine кратко отдавал native HWND в `(0,0)` (`clientToScreen0: 0,0`) до следующего pin-таймера, из-за чего интерфейс мог мигнуть или прыгнуть. Добавлен `_set_web_view_geometry_stable()`: после каждого проблемного `QWebEngineView.setGeometry()` сразу синхронно выполняется `_sync_actual_web_fit_factor()`, native-chain pin, reset/clear clip и повторный pin, ещё до возврата в event loop. На этот путь переведены `resizeEvent`, DPI-refresh после переноса между экранами, high-DPI surface rebuild, WebEngine recreate/show и вход/выход из geometry-only Web fullscreen. Live-pin во время удержания ЛКМ ускорен с `0.28s` до `0.10s`, чтобы WebEngine оставался приклеенным к родителю во время самого переноса. `QWebEngineView.setZoomFactor()` остаётся `1.0`, CSS `zoom`/`--proekt1-dpi-fit` и fullscreen down-fit не менялись. Дополнительно убрано предупреждение `QSystemTrayIcon::setVisible: No Icon set`: tray icon теперь использует уже существующую sparkle-иконку окна вместо пустого `QIcon()`.

- 2026-06-01 (фикс 200% размера интерфейса во fullscreen на втором экране): UI — `appPROEKT1/PROEKT1.py`. По свежему `appPROEKT1/ui-debug.log` найден сценарий поломки: после выбора `interface_scale=200` на `\\.\DISPLAY1` окно корректно подгонялось до `1088×752`, но следующий клик `#btn-win-max` переводил Web fullscreen в `1280×800` и fullscreen-CSS сбрасывал `transform` на один `translate(-50%, -50%)`. Из-за этого `.app-window` с `zoom:2` рисовался как `2200×1520` без forced down-fit и выходил за экран. Fullscreen-правило теперь сохраняет `scale(var(--proekt1-dpi-fit, 1))` вместе с центровкой, поэтому `100/125/150/200%` используют тот же чёткий CSS `zoom` и тот же down-fit, если выбранный размер не помещается на текущем мониторе. `QWebEngineView.setZoomFactor()` остаётся `1.0`, native pin/mixed-DPI логика и отключённый edge-resize не изменялись.

- 2026-06-01 (фиксированный размер интерфейса в настройках): UI — `appDESIGN/design-preview.html`, `appPROEKT1/PROEKT1.py`. Во вкладке WebEngine-настроек раздел `ЛОКАЛИЗАЦИЯ И ТЕМЫ` переименован в `ИНТЕРФЕЙС`; после выбора темы добавлен dropdown `Размер интерфейса` с фиксированными значениями `100%`, `125%`, `150%`, `200%` (по умолчанию `100%`). Настройка передаётся через WebUI snapshot/draft как `uiScale`, сохраняется в `config.json` ключом `interface_scale` и применяется сразу после сохранения. Для чёткости увеличения `QWebEngineView.setZoomFactor()` оставлен `1.0`: top-level окно пересчитывается от базовых `1100×760`, HTML масштабируется через Chromium CSS `zoom` на `.app-window`, а `transform:scale(...)` используется только как вынужденный down-fit, если выбранный размер не помещается в рабочую область текущего монитора. DPI-fit, mixed-DPI screen selection, fullscreen и отключённый edge-resize сохранены.

- 2026-06-01 (edge-resize временно отключён после проверки свежего UIDBG): UI — `appPROEKT1/PROEKT1.py`. Свежий `appPROEKT1/ui-debug.log` после лёгкого debug подтвердил улучшение: `debug.enabled deep=false`, точные UIDBG-теги `DOM`/`VISUAL`/`BURST`/`JS.*` отсутствуют, `web.recreate=0`, `renderProcessTerminated=0`, реальных `.error`/`exception` тегов нет. Размер лога за тест стал около `3.6 MB` при `3103` строках: строки заметно компактнее, но их много из-за интерактивного edge-resize (`native_resize=44`, много `resizeEvent`/`STATE`, `web.rgn.apply.done=309`). В этом тесте `\\.\DISPLAY1` не встречался, поэтому свежий лог проверяет в основном плавность/resize на `SyncMaster`, а не второй монитор. По просьбе пользователя функция растягивания за края временно убрана: из runtime-JS удалены невидимые edge-зоны, cursor resize и `bridge.windowAction('resize:<edge>')`; из Python удалены обработка `resize:*`, `_start_native_window_resize()` и `native_resize`/`nativeDragKind`-ветка polling. Native drag за header, fullscreen, mixed-DPI pin/verify и быстрый/deep UIDBG оставлены без изменений.

- 2026-06-01 (быстрый UIDBG и native resize после свежих mixed-DPI логов): UI — `appPROEKT1/PROEKT1.py`. По свежему `appPROEKT1/ui-debug.log` подтверждено, что переносы между `SyncMaster` и `\\.\DISPLAY1` больше не приводят к `web.recreate`/`renderProcessTerminated`, а fullscreen-enter выбирает второй монитор корректно (`web.fullscreen.enter.target` указывает на `\\.\DISPLAY1` и native chain pin даёт DPR2-размер `2560×1600`). Основной оставшийся хвост — тяжёлый штатный debug: за короткий тест накопилось около `8.8 MB` UIDBG из `STATE`/`DOM`/`VISUAL`/burst-снимков и частых live-pin логов, что само может ухудшать плавность интерфейса. Обычный `PROEKT1_UI_DEBUG=1` переведён в быстрый режим: `STATE` стал компактным, `DOM`/`VISUAL`/burst и JS resize/focus/error/MutationObserver события включаются только через `PROEKT1_UI_DEBUG_DEEP=1` (или verbose/deep значение `PROEKT1_UI_DEBUG`). Live native pin во время удержания ЛКМ оставлен для надёжности mixed-DPI WebEngine, но его throttle увеличен до `0.28s`, а poll-pin логи стали компактными. Дополнительно добавлен аккуратный frameless resize без визуальных изменений: runtime-JS создаёт невидимые 8px зоны по краям viewport и отправляет `bridge.windowAction('resize:<edge>')`, Python запускает `windowHandle().startSystemResize(Qt.Edge...)`, а на Windows имеет fallback через `WM_NCLBUTTONDOWN` с `HTLEFT/HTRIGHT/HTTOP/...`. Во fullscreen resize отключён; drag/fullscreen/pin поведение сохранено.

- 2026-06-01 (правила frameless drag/resize/QWindow после изучения гайдов): Документация — `PROEKT1 rools.md`. Изучены Qt for Python `QWindow` и StackOverflow 71604670 про draggable/resizable frameless окно. В правила приложения добавлен раздел `10.15`: интерактивное перемещение делать через `windowHandle().startSystemMove()` с ручным `move()` только как fallback; интерактивный resize — через `windowHandle().startSystemResize(edges)`; при `QSizeGrip`/ручном drag обязательно хранить drag state как `None`/active only, сбрасывать в `mouseReleaseEvent` и не двигать окно по `MouseMove`, который пришёл от parent после системного resize без собственного `MousePress`. Зафиксированы важные для нашего mixed-DPI WebEngine окна ограничения: не менять top-level geometry синхронно во время native move/resize, выбирать экран по фактическому центру `frameGeometry()`/`geometry()` через `QApplication.screenAt()`, считать `windowHandle().screen()` потенциально stale, не использовать `setScreen()` без крайней необходимости, учитывать смену `devicePixelRatio()`, держать `QWebEngineView.setZoomFactor(1.0)`, осторожно вызывать `winId()`, а `setMask()`/HRGN не считать заменой прозрачной отрисовки и CSS clip-path.

- 2026-05-29 (fullscreen и DPI-refresh после mixed-DPI drag берут фактический экран): UI — `appPROEKT1/PROEKT1.py`. Свежий `ui-debug.log` оказался коротким (`275` строк) и не содержал описанных нескольких drag-циклов, зато показал регрессию fullscreen: при запросе fullscreen окно разворачивалось в `0,0 1920×1200` на `SyncMaster`, после чего запускался старый 100%-DPI restore cascade (`force_native_redraw`, 1px nudge). Это объясняет жалобу, что fullscreen перестал работать на втором экране: `_on_web_fullscreen_requested()` выбирал экран через `self.screen()`, а на Windows после mixed-DPI переносов `windowHandle().screen()`/`self.screen()` может оставаться stale. Добавлен `_current_window_screen()`, который сначала берёт `QApplication.screenAt(frameGeometry().center())`, затем `geometry().center()`, и только потом fallback на `windowHandle().screen()`/primary. Fullscreen-enter теперь логирует `web.fullscreen.enter.target` с целевым экраном и геометрией, использует этот экран для `setGeometry(screen.geometry())`, а на high-DPI планирует native pin (`web.fullscreen.enter`). `_screen_dpi_scale()` и `_refresh_dpi_dependent_layout()` также переведены на `_current_window_screen()`, чтобы DPI/pin решения после переноса принимались по фактическому положению окна, а не по stale handle-screen. На выходе из high-DPI fullscreen добавлен `web.fullscreen.exit.high_dpi` pin+verify.

- 2026-05-29 (дожатие mixed-DPI WebEngine после нескольких переносов): UI — `appPROEKT1/PROEKT1.py`. Повторный пользовательский тест показал прогресс: после нескольких переносов интерфейс уже восстанавливается, но первые кадры после release могут быть смещены, при движении уже на `\\.\DISPLAY1` WebEngine снова остаётся в monitor-local координатах, а при возврате назад на секунду видна дублирующая рамка. Лог подтвердил две причины хвоста. Первая: `web.native_chain.pin` реально чинит позицию (`web=142,28 → 2062,-175` / `176,0 → 2096,-203`), но `web.native_surface.verify` иногда давал ложный `ok=false`, потому что `WindowFromPoint` по физическому центру `webHwnd` на DPR=2 попадал в системный `XamlWindow`, хотя native `webHwnd.windowRect` уже совпадал с `MainWindow`. Вторая: при drag внутри уже выбранного high-DPI экрана не было `screenChanged`/factor-change, поэтому surface rebuild не запускался и child HWND мог оставаться monitor-local (`main=2104,-203`, `web=184,0`) весь burst. Исправлено: добавлен общий `_web_surface_needs_native_pin()` и `_schedule_web_view_native_pin_repair()`; high-DPI pin теперь запускается во время удержания ЛКМ с throttle, сразу после любого `native_drag.release_detected`, при high-DPI `dpi.refresh` без rebuild, а также расширен таймером `320ms` после rebuild/recreate. `web.native_surface.verify` теперь считает поверхность исправной не только по `WindowFromPoint`, но и по native-геометрии: `webHwnd.root == MainWindow`, позиция `webHwnd.windowRect` совпадает с `MainWindow.clientToScreen0/windowRect`, а размер — с client/window rect. Это убирает лишний recreate на уже исправленном WebEngine и должно сократить окно “сломалось → стабилизировалось” до короткого native pin.

- 2026-05-29 (mixed-DPI WebEngine позиционирование после recovery): UI — `appPROEKT1/PROEKT1.py`. Новый тест подтвердил вывод пользователя: интерфейс не перестал рисоваться, а уезжал в левый верхний угол/на первый монитор. В логе после release старый `QWebEngineView` сначала кратко получал правильный native rect `webH=2066,-185 2176×1504`, но сразу после high-DPI `_force_web_view_surface_rebuild()` Qt geometry nudge переводил child HWND в monitor-local координаты `webH=146,18 2176×1504`; `central/webView` в `VISUAL` начинали сэмплиться с `SyncMaster`, а `WindowFromPoint(webView.center)` попадал в `ConsoleWindowClass`/чужой Chromium. Recreate создавал новый WebEngine уже в тех же неверных координатах, поэтому был виден «кусок» интерфейса слева. Исправлено: high-DPI rebuild больше не делает 1px `QWebEngineView.setGeometry()` nudge; вместо этого добавлен `web.native_chain.pin`, который проходит Win32 parent-chain от `web_view.winId()` до `MainWindow` и через `SetWindowPos(..., 0,0,parentClientW,parentClientH, SWP_NOZORDER|SWP_NOACTIVATE|SWP_NOOWNERZORDER)` прижимает каждый native child HWND к клиентской области родителя. `web.native_surface.verify` теперь проверяет центр по native `webHwnd.windowRect`, а не по сломанному Qt `mapToGlobal`, сначала пытается `verify_repair:*` pin и только если после pin hit-test всё ещё не root текущего окна — запускает `QWebEngineView` recreate. Pin также ставится после recreate install/show и несколькими короткими таймерами, чтобы перебить асинхронный Qt/Windows monitor-local move.

- 2026-05-29 (root-cause mixed-DPI gray: native HWND WebEngine оторвался от окна): UI — `appPROEKT1/PROEKT1.py`. Новый `VISUAL`-лог показал, что после переноса на `\\.\DISPLAY1` DOM остаётся здоровым, но реальный Win32 слой WebEngine ломается: Qt-геометрия окна стабильна `1088×752`, а `webHwnd` у `QWebEngineView` после release имеет `WindowRect` вроде `226,36,2176×1504`, `WindowFromPoint` в центре `webView` попадает в `ConsoleWindowClass`/desktop вместо Chromium, `webHwndTree=0`, пиксельные пробы main/webView дают чёрно-серую подложку (`#000000`/`#0c0c0c`) при живом DOM. Это подтверждает, что проблема не в CSS/repaint, а в рассинхронизации native HWND после mixed-DPI drag. Добавлен проверяемый recovery: после high-DPI `_force_web_view_surface_rebuild()` через `260/760мс` выполняется `web.native_surface.verify` (`WindowFromPoint` в центре webView должен иметь root HWND текущего MainWindow). Если root не совпадает, запускается `web.recreate.start`: старый `QWebEngineView` скрывается/отцепляется/`deleteLater()`, создаётся новый `_WebEngineChromeView`, заново подключаются `QWebChannel`, fullscreen/render-process сигналы, загружается `design-preview.html`; для recreate `_finish_startup_overlay()` имеет отдельную ветку `web.recreate.finish_show`, которая показывает новый WebEngine без startup-guard, синхронизирует DPI-fit, settings/metrics/curves snapshot и повторно проверяет native surface. Это первый фикс, который лечит именно оторванный HWND, а не пытается оживить уже сломанный Chromium surface.

- 2026-05-28 (расширение UIDBG native/visual диагностики mixed-DPI gray): UI — `appPROEKT1/PROEKT1.py`. После теста с `force_web_view_surface_rebuild.high_dpi_soft` проблема сохранилась, при этом DOM по-прежнему полностью `visible/complete`, геометрия стабильна `1088×752`, а Win32 redraw уже отсутствует. Для следующего лога добавлен новый UIDBG-событийный тип `VISUAL`: снимает реальные пиксели с экрана через `QScreen.grabWindow()` для main/central/`QWebEngineView`/startup-overlay, пишет 9 контрольных цветов, усреднение/дисперсию яркости и доминирующие `rgb16` bucket'ы; делает Win32 `WindowFromPoint` hit-test в центре и углу main/webView; расширяет HWND-инфо (`className`, visible/enabled, parent/root, style/exStyle, `ClientToScreen(0,0)`, DWM cloaked, region) и на ключевых точках пишет дерево child HWND у WebEngine. `VISUAL` вызывается на release, release-burst, screenChanged-burst, startup-overlay reveal и вокруг `_force_web_view_surface_rebuild()`. Также high-DPI `_force_web_view_surface_rebuild()` больше не вызывает `_refresh_web_view_surface()`/opacity JS-nudge вообще: для DPR>1 остаются только Qt geometry nudge, CSS fit sync и `update()`, чтобы следующий лог проверил, был ли виноват последний JS opacity repaint.

- 2026-05-28 (high-DPI WebEngine rebuild без Win32 hammer): UI — `appPROEKT1/PROEKT1.py`. Свежий `[UIDBG]`-лог после предыдущей правки показал, что delayed repaint-kicks и fullscreen geometry-nudge больше не повторяются, но единственный `_force_web_view_surface_rebuild()` после release всё ещё вызывал `_force_web_view_native_redraw()` и `_force_native_redraw()`; в момент `force_web_view_native_redraw.win32.done` у child `QWebEngineView` HWND появлялся подозрительный `windowRect` в координатах offset второго монитора (`x=164,y=56`) вместо стабильной virtual-screen позиции, после чего пользователь снова видел серый слой. Для DPR>1 / active DPI-fit rebuild теперь работает мягко: только Qt-geometry nudge `QWebEngineView`, синхронизация CSS fit и `update()`, без `SetWindowPos(SWP_FRAMECHANGED)` / `RedrawWindow` по WebEngine и main HWND. Выход из Web fullscreen на high-DPI также больше не вызывает общий `_refresh_web_view_surface()` перед soft-restore, чтобы не делать opacity repaint-nudge сразу после возврата Chromium из fullscreen; тяжёлые Win32 redraw-пути сохранены только для 100% DPI.

- 2026-05-28 (уменьшен post-release/post-fullscreen flicker на DPR=2): UI — `appPROEKT1/PROEKT1.py`. Новый `[UIDBG]`-лог после теста показал, что обычный перенос теперь держит стабильные `native_drag.release_burst` до `+17.6s`, без повторных `_force_web_view_surface_rebuild()`, но последний отложенный `_refresh_web_view_surface()` всё ещё срабатывал примерно через `0.5s` после release и мог снова гасить уже восстановленную Chromium-поверхность. Поэтому в `_refresh_dpi_dependent_layout()` после mixed-DPI release теперь остаётся только один немедленный `_force_web_view_surface_rebuild()`; все delayed repaint-kicks `80/260/450мс` убраны. Отдельно лог fullscreen на `\\.\DISPLAY1`/DPR=2 показал выход из fullscreen с серией `_force_native_redraw()` и 1px top-level `_force_geometry_nudge()` (`1088↔1089`), что объясняет дополнительные мерцания. Для high-DPI/DPI-fit выхода из Web fullscreen добавлен soft-restore путь: синхронизация DOM fullscreen-state + reset/refresh без повторных `SWP_FRAMECHANGED` и без top-level geometry-nudge; старый тяжёлый fullscreen-cascade оставлен только для 100% DPI, где он нужен для очистки stale corner pixels.

- 2026-05-28 (фикс серого фона через ~0.5с после успешного mixed-DPI переноса): UI — `appPROEKT1/PROEKT1.py`. Повторный анализ `[UIDBG]`-лога подтвердил, что после предыдущей правки окно больше не схлопывается (`native_drag.release_detected` → `1100×760`, затем корректный `dpi.apply.resize` до `1088×752`, `dpiFit≈0.989`), а DOM/WebEngine остаются `visible/complete`. Серый фон возвращался уже после нормального кадра из-за пачки отложенных тяжёлых `_force_web_view_surface_rebuild()`/native-redraw вызовов: особенно поздний rebuild около `+0.59с` после отпускания ЛКМ снова дёргал геометрию `QWebEngineView` и Win32 redraw на уже восстановленной Chromium-поверхности. В `_refresh_dpi_dependent_layout()` оставлен один тяжёлый rebuild сразу после финального resize, а последующие задержанные действия на `80/260/450мс` заменены на лёгкий `_refresh_web_view_surface()` без geometry-nudge и без повторного `SWP_FRAMECHANGED`; это должно сохранить оживляющий первый rebuild, но не сбивать уже показанный интерфейс.

- 2026-05-28 (фикс mixed-DPI схлопывания окна после отпускания на втором мониторе): UI — `appPROEKT1/PROEKT1.py`. По `[UIDBG]`-логу `appPROEKT1/ui-debug.log` найдено, что после `native_drag.release_detected` на `\\.\DISPLAY1` (`dpr=2.0`) `_refresh_dpi_dependent_layout()` вызывал `_apply_dpi_window_metrics(resize_window=True)`, а `_dpi_fit_factor_for_screen()` принудительно добавлял множитель `1 / DPR`; из-за этого окно сразу ресайзилось `1100×760 → 550×380`, CSS получал `--proekt1-dpi-fit=0.5`, а Chromium/WebEngine оставался живым, но визуально пользователь видел только серую Qt-подложку. Расчёт DPI-fit изменён: Qt/Chromium уже учитывают DPR монитора, поэтому размер окна теперь подгоняется только под доступную логическую область экрана (`availableGeometry`), без отдельного `1/DPR`-схлопывания. Для текущего второго экрана это означает фактор около `752/760≈0.989`, а не `0.5`; WebEngine не получает резкий half-size rebuild после отпускания drag.

- 2026-05-28 (UI-debug телеметрия для диагностики mixed-DPI/WebEngine): UI — `appPROEKT1/PROEKT1.py`. Добавлен временный отладочный режим, включённый по умолчанию (`PROEKT1_UI_DEBUG=1`, отключение: `PROEKT1_UI_DEBUG=0`), который печатает в pwsh строки с префиксом `[UIDBG ...]`. Логируются: геометрия `QMainWindow`/centralWidget/`QWebEngineView`, min-size, `devicePixelRatioF`, текущий/целевой DPI-fit, флаги `_screen_surface_rebuild_pending`, `_native_drag_active`, `_web_fs_active`, состояние ЛКМ через Qt и WinAPI `GetAsyncKeyState`, все `QScreen` (name/model/geometry/available/DPI/DPR), Win32 HWND main/web (`GetWindowRect`, `GetClientRect`, `GetDpiForWindow`, `GetWindowRgn`). Инструментированы ключевые события: `showEvent`, `moveEvent`, `resizeEvent`, установка/снятие WebEngine HRGN, `_refresh_web_view_surface`, `_force_native_redraw`, `_force_web_view_native_redraw`, `_force_web_view_surface_rebuild`, WebEngine `loadStarted/loadProgress/loadFinished/urlChanged/titleChanged/renderProcessTerminated`, `_finish_startup_overlay`, `_apply_dpi_window_metrics`, `_sync_actual_web_fit_factor`, `_sync_web_dpi_fit_factor`, `_on_screen_dpi_changed`, `_refresh_dpi_dependent_layout`, `_on_screen_changed`, native drag start/release/poll. Через `QWebChannel` добавлен slot `debugLog(tag,payload)`, а runtime-JS отправляет DOM-снапшоты и события `window.resize/focus/blur`, `visibilitychange`, `error`, `unhandledrejection`, MutationObserver по `html/body/#app-window` (`style/class`). DOM-снапшоты содержат viewport/DPR, CSS-переменные `--proekt1-dpi-fit*`, body classes/rect/computed style, `.app-window` rect/offset/client/scroll/inline/computed style. При `screenChanged` и отпускании native drag запускается burst-сэмплер (16 снимков по 200мс), чтобы увидеть, в какой момент после переноса WebEngine превращается в серый фон.

- 2026-05-28 (стабилизация переноса окна >50% на mixed-DPI монитор): UI — `appPROEKT1/PROEKT1.py`. По скринам выяснилось, что после пересечения ~50% второго экрана окно переключается на другой `QScreen`/DPI ещё ДО отпускания ЛКМ: внешний Qt-фрейм меняет фактический размер/масштаб, а HTML уже получает `dpi-fit-active` и ужимается в 0.5, из-за чего справа/снизу появляется огромная тёмная пустая область, а интерфейс остаётся узкой полосой. Исправлено на уровне причины: (1) CSS `--proekt1-dpi-fit` теперь синхронизируется не с целевым DPI экрана напрямую, а с фактическим размером `QWebEngineView` (`_actual_web_fit_factor`: min(width/1100, height/760, 1)). Пока окно физически ещё 1100×760 во время drag — HTML остаётся scale=1 и не ужимается преждевременно; после реального resize до 550×380 на 200%-мониторе scale становится 0.5 и дизайн снова заполняет область. (2) Добавлен `_native_drag_active` + WinAPI `GetAsyncKeyState(VK_LBUTTON)` (`_is_left_mouse_button_down`) — `QApplication.mouseButtons()` во время Windows system move может уже не видеть ЛКМ, поэтому прежний debounce иногда считал drag завершённым и запускал `_apply_dpi_window_metrics(resize_window=True)` прямо пока пользователь держит окно. Теперь `_refresh_dpi_dependent_layout` не меняет размер/scale до фактического отпускания ЛКМ; `_poll_native_drag_release` запускает DPI-refresh только после отпускания. (3) На high-DPI экране `_apply_web_view_native_round_rgn` всегда очищает native HRGN у `QWebEngineView` (CSS clip-path уже даёт 24px скругление), чтобы `SetWindowRgn` не гасил Chromium-поверхность при смене монитора.

- 2026-05-28 (WebEngine пропадал после пересечения середины второго монитора): UI — `appPROEKT1/PROEKT1.py`. После оживления интерфейса остался сценарий: до середины переноса на второй экран всё видно, после пересечения середины Windows/Qt переключают окно на другой `QScreen`, и Chromium-поверхность снова пропадает (остаётся только тёмный Qt-фрейм). Причина: оптимизация предыдущего фикса запускала тяжёлый redraw cascade только при изменении `dpi_fit_factor`; но сам факт `screenChanged` может убить swap-chain даже если factor уже был рассчитан заранее или визуально не успел измениться. Исправлено: добавлен флаг `_screen_surface_rebuild_pending`, который выставляется в `_on_screen_changed`; там же сразу снимается native HRGN с `QWebEngineView` через `_clear_web_view_native_round_rgn()`, чтобы регион не occluded Chromium во время переключения монитора. После отпускания мыши `_refresh_dpi_dependent_layout` запускает rebuild не только при изменении factor, но и при pending screen-change: новый helper `_force_web_view_surface_rebuild()` настраивает прозрачный фон, возвращает `web_view` в `cw.rect()`, делает 1px geometry-nudge только для `QWebEngineView`, синхронизирует CSS DPI-fit, вызывает opacity repaint-нудж и native redraw для web_view HWND + MainWindow. Повторные kicks запланированы на 40/160/260/450мс. Стартап не тормозится, потому что cascade не запускается без `screenChanged` или изменения factor.

- 2026-05-28 (.app-window всегда покрывает viewport + redraw-cascade только при смене DPI): UI — `appPROEKT1/PROEKT1.py`. После предыдущих фиксов офсет внутри Qt-фрейма всё равно проявлялся (контент сдвинут вниз-вправо, sidebar частично за краем), и интерфейс грузился заметно дольше. Причины и исправления: (1) В runtime-CSS базовое правило для `.app-window` оставляло оригинальный `position:fixed; left:calc(50% - 550px); top:calc(50% - 380px); width:1100px; height:760px` — при любом отличии web_view от 1100×760 (после ресайза/drag) calc уходит в минус и контент смещается, оставляя серую область фрейма видимой. Базовое правило теперь жёстко прижимает `.app-window` к (0,0) с `width:max(1100px,100vw)!important; height:max(760px,100vh)!important; transform:none!important` — гарантировано покрытие всего web_view без зависимости от точного размера. DPI-fit оверрайд (`.dpi-fit-active`) добавляет только `width:1100px; height:760px; transform:scale(var(--proekt1-dpi-fit))` для масштабирования дизайна вниз на 200%-мониторе. (2) Добавлен немедленный `_wipe()` inline-стилей при bind MutationObserver на `#app-window` — observer ловит только БУДУЩИЕ мутации, поэтому если HTML's initDrag успел сработать ДО запуска runtime JS (race на загрузке), оставшиеся inline `left/top` остались бы. (3) Тяжёлый redraw cascade `_force_native_redraw + QTimer.singleShot 40/80/220/450` в `_refresh_dpi_dependent_layout` теперь запускается ТОЛЬКО когда DPI-factor реально изменился (`abs(new-prev)>1e-3`). На старте `showEvent` дважды вызывает refresh при том же DPI = было до 10 timer kicks впустую с тяжёлыми Win32+Chromium операциями. Теперь только при реальном переносе между мониторами с разным DPI.

- 2026-05-28 (HTML preview drag «двигал» .app-window внутри окна — фикс): UI — `appPROEKT1/PROEKT1.py`. При перетаскивании окна за header HTML-контент внутри Qt-фрейма съезжал вниз-вправо и обрезался. Причина: в `appDESIGN/design-preview.html` (line 3107, `initDrag` IIFE) на `#header.mousedown` навешен обработчик, который ставит `dragging=true` и потом на каждом `mousemove` пишет в `.app-window.style.left/top`. Наш runtime native-drag слушает то же событие в capture-фазе и вызывал `event.stopPropagation()` — но это останавливает только bubble к ПРЕДКАМ; target-фаза на том же элементе всегда вызывает ВСЕ слушатели (capture/bubble флаги для target-listener не учитываются). Поэтому оба обработчика срабатывали: наш запускал Win32 OS-drag, HTML-овский параллельно двигал внутренний `.app-window` через inline-стили. Заменено на `event.stopImmediatePropagation()` — гарантированно блокирует sibling-слушателей на том же узле. Дополнительно навешан `MutationObserver` на `.app-window` (attributeFilter: ['style']), который очищает любые установленные inline `left/top` (defense-in-depth, если HTML-обработчик всё-таки успеет добраться до элемента до bind runtime).

- 2026-05-28 (DPI-fit: возврат к transform:scale с фиксированными 1100×760, отказ от CSS zoom): UI — `appPROEKT1/PROEKT1.py`. На втором мониторе с DPI 200% (factor=0.5) при отпускании ЛКМ был виден «БЛИНК» полного интерфейса на 0.1с → дизайн пропадал, оставался серый прямоугольник, занимающий ~всю площадь web_view. Причина: CSS `zoom: 0.5` на `body` в Chromium НЕ меняет значения `vw/vh` — `.app-window { width:100vw; height:100vh }` оставалась логически = размер web_view (550×380), но рендерилась в 50% = 275×190, освобождая половину окна, где торчала серая подложка `_RoundedWindowFrame`. Решено возвратом к `transform: scale(var(--proekt1-dpi-fit, 1))` на `.app-window` с фиксированными `width:1100px; height:760px; left:0; top:0; transform-origin:0 0`. Теперь при любом DPI: `.app-window` всегда 1100×760 дизайна, масштабируется через GPU-transform к физическому размеру web_view (например, 550×380 на 200% DPI), полностью заполняет область, никакой серой подложки. Правило `body.dpi-fit-active { zoom }` оставлено пустым no-op для совместимости — transform теперь читает CSS-переменную напрямую, переключение класса не требуется. `will-change:clip-path, transform` подсказывает Chromium держать GPU-слой.

- 2026-05-28 (пустой WebEngine на втором мониторе после переноса): UI — `appPROEKT1/PROEKT1.py`. После предыдущей правки (дебаунс ресайза до отпускания ЛКМ) краш исчез, но на втором мониторе оставался пустой серый прямоугольник вместо UI. Причина: `_refresh_web_view_surface()` запускался из `_on_screen_changed` через 450/750мс — но ресайз теперь происходит ПОЗЖЕ (после отпускания мыши через debounce-timer), и эти ранние таймеры кикали ещё старую Chromium-поверхность. Перенесено: surface refresh теперь триггерится из самого конца `_refresh_dpi_dependent_layout` (т.е. ПОСЛЕ применения новых метрик), и дополнительно перед ним вызывается тяжёлый `_force_native_redraw()` (`SetWindowPos(SWP_FRAMECHANGED)` + `RedrawWindow(RDW_INVALIDATE|RDW_FRAME|RDW_ALLCHILDREN|RDW_UPDATENOW|RDW_ERASE)`) — он заставляет DWM и Chromium-GPU полностью пересобрать swap chain под новый DPI/экран. Последовательность: `_force_native_redraw()` сразу, +40мс ещё раз, +80/220/450мс — `_refresh_web_view_surface()` (opacity-нудж). Без `_force_native_redraw` opacity-нудж не помогал, потому что Chromium не знал, что его HWND переехал на другой дисплей.

- 2026-05-28 (фикс бесконечной петли setGeometry при перетаскивании между мониторами): UI — `appPROEKT1/PROEKT1.py`. При полном переносе окна на второй монитор с другим DPI приложение зависало в петле тысячами строк `QWindowsWindow::setGeometry: Unable to set geometry 1100x760+818+1 ... Resulting geometry: 1100x760+819+0` и крашилось. Причина: `_on_screen_changed` синхронно вызывал `_apply_dpi_window_metrics(resize_window=True)` + `resize()` + `move()` + `_refresh_dpi_dependent_layout()` (который снова делал ресайз и звал `ensure_visible()` с ещё одним resize+move). Каждое такое движение во время активного OS-drag сдвигало центр окна обратно через границу мониторов → новый `screenChanged` → каскад. Исправлено: (1) в `_apply_dpi_window_metrics` добавлен re-entry guard `_dpi_apply_in_progress`; setMinimumSize сначала понижается, чтобы resize до меньшего размера не клиппился прежним минимумом, потом восстанавливается. (2) `_on_screen_dpi_changed` перенесён на дебаунс через единый `QTimer` (200мс) — серия сигналов схлопывается в один отложенный refresh. (3) `_refresh_dpi_dependent_layout` проверяет `QApplication.mouseButtons() & LeftButton`: пока пользователь тащит окно, refresh откладывается ещё на 200мс. (4) Из `_refresh_dpi_dependent_layout` убран вызов `ensure_visible()` (его собственный resize+move был частью цикла; `ensure_visible` остаётся доступен из `showEvent` / `screenChanged`). (5) `_on_screen_changed` упрощён — никаких immediate resize/move, всё делегировано в дебаунсер.

- 2026-05-28 (DPI-fit через CSS zoom + восстановление 24px clip-path → язычки и пустой экран на 200% DPI): UI — `appPROEKT1/PROEKT1.py`. Подход с `transform:scale(...)` + `transform-origin:0 0` на `.app-window` ломал разметку (контент рендерился без масштаба, торчал из viewport только верхний-левый угол на 200% мониторе) и при factor=1.0 переопределял `transform` родного HTML, отчего у углов снова появлялись «язычки». Заменено на `body.proekt1-runtime.dpi-fit-active { zoom: var(--proekt1-dpi-fit, 1); }` — Chromium-native zoom пересчитывает viewport для дочерних элементов, `.app-window` остаётся `100vw × 100vh` в логических координатах body, никаких override width/height/transform. Базовое правило `.app-window` восстановлено в рабочее состояние: `clip-path:inset(0 round 24px)` + `will-change:clip-path` (без этих свойств края HTML-окна не совпадают с 24px Qt-фреймом → видимы «язычки» по углам). CSS-переменные `--proekt1-dpi-fit-w/h` больше не используются (оставлены в `:root` для обратной совместимости). В `_on_screen_changed` добавлены отложенные вызовы `_refresh_web_view_surface()` (через 250 и 500 мс) — Chromium-compositor успевает завершить ресайз swap chain до repaint-нуджа. Repaint-нудж переделан с `style.transform='translateZ(0)'` на `style.opacity='0.9999'` (предыдущий вариант перебивался `transform:... !important` из runtime-CSS и compositor не кикался).

- 2026-05-28 (DPI-fit окна на мониторах 150/200%): UI — `appPROEKT1/PROEKT1.py`. Исправлен случай, когда при перетаскивании окна на монитор с масштабом Windows 200% внешний `QMainWindow` становился физически в 2 раза больше (`1100×760` logical → `2200×1520` physical), а WebEngine/HTML оставался видимым только частично. Добавлены базовые размеры дизайна (`1100×760`, min `960×660`) и DPI-fit фактор: `min(1 / screen DPI scale, availableWidth / 1100, availableHeight / 760)`. При смене экрана/геометрии окно ресайзится в логических координатах на этот фактор, а HTML `.app-window` масштабируется через CSS-переменные `--proekt1-dpi-fit`, `--proekt1-dpi-fit-w`, `--proekt1-dpi-fit-h` (например, при factor `0.5`: `width:200vw`, `height:200vh`, `transform:scale(0.5)`). `QWebEngineView.setZoomFactor()` принудительно оставлен `1.0`, потому что zoom `<1` на смешанных DPI мог оставлять пустой серый Chromium-слой. `_RoundedWindowFrame` и Win32 HRGN-радиус масштабируются тем же визуальным фактором. Это держит одинаковый физический размер интерфейса на 100/150/200% и не даёт окну вылезать за рабочую область второго экрана.

- 2026-05-28 (пустой WebEngine на DPI-fit): UI — `appPROEKT1/PROEKT1.py`. Исправлен сценарий, когда после переноса окна на монитор 200% оставалась только серая/графитовая Qt-подложка без HTML-интерфейса. Причина: даже top-level `SetWindowRgn` на `QWebEngineView` может заставить Chromium перестать рисовать после mixed-DPI resize/move. Добавлен `_clear_web_view_native_round_rgn()`, а `_apply_web_view_native_round_rgn()` теперь при активном DPI-fit (`_dpi_fit_factor < 0.999`) полностью сбрасывает Win32 region у WebEngine и возвращается без установки HRGN. Дополнительно region не ставится, пока `web_view` скрыт за startup-overlay: до первого `web_view.show()` он только сбрасывается, а после показа `_finish_startup_overlay()` заново применяет DPI-метрики, clip-режим и refresh surface. В этом режиме скругление обеспечивает CSS `.app-window` (`border-radius`/`clip-path`) поверх `_RoundedWindowFrame`, а Chromium остаётся без native clipping.

- 2026-05-28 (перетаскивание окна за HTML/WebEngine-шапку): UI — `appPROEKT1/PROEKT1.py`. Добавлен native-drag для видимой шапки `#header` внутри `QWebEngineView`: runtime-JS через `QWebChannel` отправляет `bridge.windowAction('drag')` при ЛКМ по шапке, исключая `.win-btn` и fullscreen-режим. На Python-стороне `_on_web_window_action('drag')` вызывает `_start_native_window_drag()`: сначала `windowHandle().startSystemMove()`, fallback для Windows — `ReleaseCapture()` + `SendMessageW(WM_NCLBUTTONDOWN, HTCAPTION)`. Это двигает настоящий frameless `QMainWindow`, а не только HTML `.app-window`; старый preview-drag HTML при этом не мешает runtime.

- 2026-05-28 (fullscreen без масштабирования интерфейса): UI — `appPROEKT1/PROEKT1.py`, `appDESIGN/design-preview.html`. По нажатию fullscreen окно приложения по-прежнему раскрывается на экран и рисует полноэкранную подложку темы, но сам `.app-window` больше не увеличивается: `transform: translate(-50%, -50%) scale(1.2)` заменён на `transform: translate(-50%, -50%)` в runtime-CSS PySide/WebEngine и в HTML-макете. Размер интерфейса во fullscreen остаётся штатным `1100×760`, без изменения масштаба элементов.

- 2026-05-28 (theme-aware Qt-подложка приложения): UI — `appPROEKT1/PROEKT1.py`. `_RoundedWindowFrame`, который рисует видимую подложку под `QWebEngineView` и раньше оставался жёстко фиолетовым (`#161735→#121431→#0d0f28`), теперь берёт градиент и цвет рамки из `THEMES`: violet = `#161735→#121431→#0d0f28`, graphite = `#1d1e22→#16171b→#101116`, light = `#f5f6fb→#eaedf5→#dde1ec`. Добавлен `setTheme()` у `_RoundedWindowFrame`; `MainWindow` хранит `self._rounded_frame` и обновляет его при старте, после чтения `config.json`, а также при смене темы из WebEngine-настроек и старого `SettingsDialog`. Это убирает постоянный/мигающий «фиолетовый экран» между startup-overlay и основным интерфейсом: если Chromium-поверхность ещё не готова или временно обрезана, видимая подложка совпадает с текущей темой.

- 2026-05-28 (DPI-aware WebEngine на масштабах Windows 125/150/175%): UI — `appPROEKT1/PROEKT1.py`. Исправлена обрезка интерфейса при системном масштабе Windows выше 100%: `_apply_web_view_native_round_rgn()` больше не строит `SetWindowRgn(CreateRoundRectRgn(...))` только по логическим `QWidget.width()/height()`. Win32 region теперь строится по максимальному размеру из `GetClientRect(HWND)` и `logical_size * DPI`, где DPI берётся из `web_view.devicePixelRatioF()`, `windowHandle().devicePixelRatio()` и `GetDpiForWindow(HWND)`. Это важно потому, что `GetClientRect()` для Chromium/Qt HWND на некоторых режимах DPI тоже возвращает логические координаты; на 150% такой HRGN всё ещё резал WebEngine до левого верхнего куска, а справа/снизу была видна подложка `_RoundedWindowFrame`. Радиус 24px масштабируется тем же native/logical коэффициентом. Добавлены сигналы смены DPI/геометрии экрана (`logicalDotsPerInchChanged`, `geometryChanged`, `availableGeometryChanged`) с переустановкой WebEngine region, а до создания `QApplication` выставлена `HighDpiScaleFactorRoundingPolicy.PassThrough`.

- 2026-05-26 (нативное hardware-клипование Chromium-HWND через SetWindowRgn): UI — `appPROEKT1/PROEKT1.py`. Добавлен `_apply_web_view_native_round_rgn()`: на каждом `resizeEvent` и после выхода из fullscreen ставит `CreateRoundRectRgn(0,0,w+1,h+1,48,48)` через `SetWindowRgn` на верхний HWND QWebEngineView. Позже уточнено: внутренние Chromium child-HWND клиповать нельзя, иначе Chromium может считать поверхность occluded и перестать рисовать. Корневая причина: Qt per-pixel alpha скругляет только MainWindow, а Chromium-композитор рендерит OPAQUE-пиксели в прямоугольные границы своего HWND, которые после fullscreen-циклов накапливают GPU-кэш и видны как «второй слой углов». `SetWindowRgn` — hardware-клип, не зависит от GPU-состояния. Лёгкая зубчатость HRGN маскируется антиалиасингом `_RoundedWindowFrame` поверх. `_reset_web_view_clip()` теперь после `clearMask()` повторно применяет region.
