...
10.15. **Frameless window / drag / resize / mixed-DPI (строго для `MainWindow`):**
   - Для интерактивного перемещения frameless-окна всегда сначала использовать `windowHandle().startSystemMove()`; ручной `move()`/`setPosition()` — только fallback после отказа Qt/ОС. Это сохраняет нативное поведение Windows: snapping, перенос между мониторами, анимации, корректную работу window manager.
   - Для интерактивного изменения размера использовать `windowHandle().startSystemResize(edges)`, где `edges` — один край или два соседних края/угол. Не делать постоянный ручной `setGeometry()` на каждом `mouseMove`, если resize может выполнить window manager.
   - Если используется `QSizeGrip` или ручной drag-state: drag-позиция должна инициализироваться только по `LeftButton`, проверяться на `None` в `mouseMoveEvent` и обязательно сбрасываться в `mouseReleaseEvent`. `QSizeGrip` забирает мышь на время системного resize; после release родитель может получить `MouseMove` без своего `MousePress`, поэтому старый `oldPos` нельзя переиспользовать.
   - Во время активного native/system move/resize нельзя синхронно менять top-level `setGeometry()`/`resize()`/`move()` из `screenChanged`, `moveEvent`, `resizeEvent` или DPI-callback. Все DPI/layout refresh делать через debounce и только после фактического отпускания ЛКМ (`_native_drag_active` + WinAPI `GetAsyncKeyState`).
   - На Windows проверка “ЛКМ ещё зажата?” должна сначала смотреть WinAPI `GetAsyncKeyState(VK_LBUTTON)`, и только потом fallback на `QApplication.mouseButtons()`: после `startSystemMove()` Qt иногда ещё показывает `qtButtons=1`, хотя Windows уже отпустила кнопку, и из-за этого post-release repair может не стартовать.
   - После переноса между экранами debounced monitor/DPI refresh тоже не должен ресайзить top-level окно сам по себе: пользовательский выбор масштаба (`80/100/125/150/200%`) может менять размер окна, но `screenChanged`/DPI-refresh обязан сохранять текущий frame size и пересинхронизировать только WebView/native pin/mask. Иначе после отпускания ЛКМ виден скачок `1088×752` ↔ `1100×760`.
   - `QWindow.geometry()` и `frameGeometry()` живут в координатах virtual desktop своего экрана; при выборе монитора использовать фактический центр `frameGeometry().center()`/`geometry().center()` через `QApplication.screenAt()`, а `windowHandle().screen()` считать возможным stale после mixed-DPI drag.
   - `setScreen()` для уже созданного top-level `QWindow` может пересоздать native surface и сам по себе не перемещает окно на геометрию нового экрана; для нашего frameless+transparent+`QWebEngineView` окна без крайней необходимости не вызывать `setScreen()`. Для fullscreen выбирать экран по текущему центру и ставить geometry относительно `screen.geometry()`.
   - `devicePixelRatio()` окна может измениться после переноса между мониторами; DPI-fit считать после окончательной геометрии `QWebEngineView`, а не заранее по целевому DPR. `QWebEngineView.setZoomFactor()` по умолчанию держать `1.0`, но для пользовательского масштаба ниже `100%` на high-DPI/mixed-DPI разрешён `setZoomFactor(ui_scale)` вместо CSS `transform:scale(ui_scale)`: этот путь стабильнее для Chromium compositor и не даёт чёрный surface при `80%`.
   - Для прозрачного frameless `QWebEngineView` аппаратный Chromium compositor запрещён: при быстром hover/перерисовке он даёт диагональные, ступенчатые или полные блинки без `renderProcessTerminated` и без JS-крэша. До импорта/создания WebEngine выставлять `QTWEBENGINE_CHROMIUM_FLAGS` с `--disable-gpu --disable-gpu-compositing --disable-gpu-rasterization --disable-zero-copy --disable-accelerated-2d-canvas --disable-webgl`; в настройках страницы выключать `WebGLEnabled` и `Accelerated2dCanvasEnabled`. В `ui-debug.log` должна быть строка `webengine.flags`; после `showEvent`/restore применять только paint-only `_recover_web_view_after_show()` без reload и без изменения размеров окна.
   - `winId()` создаёт native window, если его ещё не было; вызывать его только когда действительно нужен HWND/WinAPI, и после этого учитывать, что native surface уже существует и может требовать pin/verify на mixed-DPI.
   - `QWindow.setMask()`/native region — только подсказка/ограничение для window manager и ввода; за прозрачность и визуально чистые края отвечает само приложение (`_RoundedWindowFrame`, CSS `clip-path`, прозрачная очистка). Для видимого normal-mode `QWebEngineView` верхний HWND должен всегда оставаться под `SetWindowRgn(CreateRoundRectRgn)`, включая native drag и mixed-DPI перенос: если HRGN снять хотя бы на время удержания ЛКМ, прямоугольный Chromium backing сразу виден как «уголки». Клипать внутренние Chromium child-HWND нельзя, только верхний `QWebEngineView` HWND.
   - При mixed-DPI переносе Win32 `GetClientRect()` у `QWebEngineView` и его native parent-chain может несколько кадров оставаться stale от прошлого DPR. Такой rect нельзя принимать вслепую для `SetWindowRgn`/`SetWindowPos`: сначала сравнить `client_w / logical_w` и `client_h / logical_h` с текущим ожидаемым DPR; если `GetDpiForWindow()` доступен, он приоритетнее временно stale `devicePixelRatioF()`/`windowHandle().devicePixelRatio()`. Если scale не совпадает, native size считать из logical WebView × current DPR, иначе при DPR=2 → DPR=1 HRGN оказывается вдвое больше видимой поверхности и углы проявляются до release.
   - Во время удержания ЛКМ/native drag нельзя делать Win32 `SetWindowPos` с изменением размера для native-chain `QWebEngineView`: на смешанных DPI это записывает физический размер (`2200×1520`/`2750×1900`) обратно в Qt logical geometry и может разогнать поверхность каскадом (`1375×950 → 2750×1900 → 5500×3800`). Исключение при активном переносе на DPR>1 — только position-only repair (`SWP_NOSIZE`, `x=0,y=0`) для child/native-chain HWND, если Windows оставила WebEngine на stale-origin до release. HRGN во время DPR>1 drag строить по фактическому текущему `GetClientRect()`, если он ещё не успел стать physical-size; на DPR=1 live-drag оставляет только `SetWindowRgn`/HRGN, обычный size-pin выполнять после release/settled verify.
   - DPI-fit и пользовательский scale считать по стабильной логической области (`centralWidget().rect()`/top-level window), а не по `web_view.width()/height()`, если WebView уже мог дрейфовать после native pin. Проверка `QWebEngineView` surface считается успешной только при совпадении Win32 geometry с main client rect; одного hit-test недостаточно, потому что hit может проходить даже при половинном или двойном native HWND.
   - После выхода из Web fullscreen запрещён top-level `1px geometry nudge`: он действительно форсит repaint, но пользователь видит краткий скачок frame. Для очистки stale pixels использовать paint-only путь: `_sync_web_fullscreen_state(False)`, `_reset_web_view_clip()`, `_refresh_web_view_surface()`, `_apply_rounded_mask()`, `_force_native_redraw()` и native pin/verify без изменения размера окна.
   - Источники правил: Qt for Python `QWindow` (`startSystemMove`, `startSystemResize`, `geometry`, `screenChanged`, `devicePixelRatio`, `setScreen`, `winId`, `setMask`) и StackOverflow 71604670 (конфликт `QSizeGrip`/parent mouse events и обязательный сброс `oldPos`).
